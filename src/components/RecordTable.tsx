...
export default function RecordTable({ records, gages, vendors }: RecordTableProps) {
  ...
  const handleDeleteSelected = async () => {
    ...
    try {
      ...
      alert(error.message || t('common.common.error_batch') || 'Delete failed');
    } finally {
      setIsDeleting(false);
    }
  };
        ...
        <div className="flex flex-col md:flex-row gap-4 justify-between items-center">
          ...
          <div className="flex gap-2 w-full md:w-auto">
            ...
            <button
              ...
              className={`px-4 py-2 rounded-xl text-sm font-bold flex items-center justify-center gap-2 transition-all ${isFilterOpen ? 'bg-kst-blue text-white shadow-lg shadow-blue-100' : 'bg-slate-50 text-slate-600 hover:bg-slate-100'
                }`}
            >
            ...
            <div className="relative">
              <button
                ...
                className={`px-4 py-2 rounded-xl text-sm font-bold flex items-center justify-center gap-2 transition-all ${isLayoutOpen ? 'bg-slate-800 text-white' : 'bg-slate-50 text-slate-600 hover:bg-slate-100'
                  }`}
              >
          ...
          <div className="sticky top-0 z-40 bg-kst-blue text-white px-6 py-3 flex items-center justify-between animate-in slide-in-from-top duration-300">
            ...
            <div className="flex gap-2">
              ...
              <button 
                ...
                disabled={isDeleting}
                className="px-4 py-2 bg-red-500 text-white rounded-xl text-sm font-bold flex items-center justify-center gap-2 hover:bg-red-600 shadow-lg shadow-red-500/20 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                <Trash2 className={`w-4 h-4 ${isDeleting ? 'animate-spin' : ''}`} /> 
                {isDeleting ? (t('common.common.processing') || '處理中...') : t('common.common.delete_selected')}
              </button>
            </div>
          </div>
        )}
                  ...
                  {columnOrder.filter(id => visibleColumns.has(id)).map(colId => {
                    switch (colId) {
                      ...
                      case 'calType':
                        return (
                          <td key={colId} className="px-6 py-4">
                            <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${record.gage.calType === '免校正' ? 'bg-slate-100 text-slate-500' : 'bg-blue-50 text-kst-blue'
                              }`}>
