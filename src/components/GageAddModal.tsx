...
import { createGageAction, getCategoriesAction } from "@/app/actions/gage-actions";
...
export default function GageAddModal() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const res = await createGageAction(data as any);
      if (res && res.success) {
        setIsOpen(false);
        router.refresh();
      } else {
  ...
  return (
    <>
      <button 
        ...
        className="flex items-center gap-2 px-4 py-2.5 bg-kst-blue text-white rounded-xl font-black hover:bg-blue-700 transition-all shadow-lg shadow-blue-100 active:scale-95"
      >
        <Plus className="w-5 h-5" />
        <span>{t('calibration.gage.add')}</span>
      </button>
          ...
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-5xl overflow-hidden border border-slate-100 animate-in fade-in zoom-in duration-200 flex flex-col h-[90vh]">
            ...
            <header className="px-8 py-5 bg-slate-50 border-b border-slate-100 flex justify-between items-center shrink-0">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-black text-slate-800 text-lg uppercase tracking-tight">{t('calibration.gage.add')}</h3>
                  <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Register new equipment to system</p>
                </div>
              </div>
            ...
            <form id="addGageForm" onSubmit={handleSubmit} className="flex-1 overflow-y-auto overflow-x-hidden p-8 space-y-10 custom-scrollbar">
              ...
              <section className="space-y-5">
                ...
                <div className="flex flex-wrap -mx-3">
                  <div className="w-full px-3 mb-5">
                    <label className={labelStyle}>{t('calibration.gage.id')}</label>
                  ...
                  <div className="w-full px-3">
                    ...
                    <input type="text" name="spec" className={inputStyle} />
                  </div>
                </div>
              </section>
